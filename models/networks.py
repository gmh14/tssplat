...
from utils.config import get_rank  # NOQA
...
def get_activation(name) -> Callable:
    ...
    if name == "none":
        ...
        return lambda x: torch.where(
            x > 0.0031308,
            torch.pow(torch.clamp(x, min=0.0031308),
                      1.0 / 2.4) * 1.055 - 0.055,
            12.92 * x,
        ).clamp(0.0, 1.0)
    elif name == "exp":
    ...
    def __init__(self, in_channels, config, dtype=torch.float32):
        ...
        with torch.cuda.device(get_rank()):
            self.encoding = tcnn.Encoding(
                in_channels, encoding_config, dtype=dtype)
        self.n_output_dims = self.encoding.n_output_dims
...
def get_encoding(n_input_dims: int, config) -> nn.Module:
    ...
    if config.otype == "ProgressiveBandFrequency":
        encoding = ProgressiveBandFrequency(
...
class VanillaMLP(nn.Module):
    def __init__(self, dim_in: int, dim_out: int, config: dict):
        ...
        layers = [
            self.make_linear(dim_in, self.n_neurons,
        ...
        layers += [
            self.make_linear(self.n_neurons, dim_out,
                             is_first=False, is_last=True)
        ]
        self.layers = nn.Sequential(*layers)
        self.output_activation = get_activation(
            config.get("output_activation", None))
...
def get_mlp(n_input_dims, n_output_dims, config) -> nn.Module:
    ...
    if config.otype == "VanillaMLP":
        network = VanillaMLP(n_input_dims, n_output_dims,
                             config_to_primitive(config))
        ...
        network = TCNNNetwork(n_input_dims, n_output_dims,
                              config_to_primitive(config))
    return network
...
def create_network_with_input_encoding(
    ...
    if encoding_config.otype in [
        ...
        network = get_mlp(encoding.n_output_dims,
                          n_output_dims, network_config)
        network_with_input_encoding = NetworkWithInputEncoding(
            encoding, network)
    else:
